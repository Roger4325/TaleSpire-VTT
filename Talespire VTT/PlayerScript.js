...
actionTableRows.forEach((row, index) => {
    ...
    if (ninthColumnCell) {
        ...
        console.log('%cChecked Options:', 'color: red', checkedOptions); // Log the checked options for the current row
    
        if (checkedOptions.length > 0) {
            // Save as an object based on the checked options
            const ninthColumnData = {};
            checkedOptions.forEach(option => {
                const matchingCheckbox = checkboxData.find(checkbox => checkbox.category === option);
                if (matchingCheckbox) {
                    ninthColumnData[matchingCheckbox.category] = true;
                }
            });
